...
# 추가 12 [패치 완료]
- 관리자 단축키 `O`를 누르면 `triggerAdminCelebration()`이 실행되어 즉시 완전 연결 축하 시퀀스를 기동합니다. 축하 오버레이를 열 때 HUD 배너를 숨기고, 닫힐 때는 기존의 스크롤락/해제 루틴과 동일하게 상태를 복원합니다.
- 핫스팟을 통해 새로운 노드를 최초로 연결하면 `showConnectionBanner()`가 2초간 `hud__banner`를 노출해 연결 상황을 안내합니다. 배너 제목은 노드 라벨과 `connected` 하이라이트(기존 SUCCESS! 스타일)를 조합해 구성하고, 본문에는 짧은 설명 문구를 제공합니다.
- 배너 문구는 `connectionBannerModule`(전역 `window.MultiverseConnectionBanner`)을 통해 노드별로 등록/재정의할 수 있습니다. 미등록 노드에는 기본 문구가 자동으로 제공되며, 등록 항목은 `{ title, highlight, body }` 옵션을 사용합니다.

# 추가 13
- 조작 안내가 오른쪽 모서리에서 슬라이드 인으로 등장하고, 다시 오른쪽 모서리로 슬라이드 아웃하여 사라집니다.
- 노드 연결 시 등장하는 연결 상황 안내 배너는 왼쪽 모서리에서 슬라이드 인으로 등장하고, 다시 왼쪽 모서리로 슬라이드 아웃하여 사라집니다.

- 모든 노드 연결 이벤트 영상 재생 팝업은 영상과 닫기 버튼만 있는 기존 다른 영상의 팝업 레이아웃과 통일합니다. 영상이 최초 1회 재생된 후, 반투명 검정 배경이 팝업 위에 깔리고 기존 팝업에 있었던 'ALL UNIVERSES CONNECTED' 문구와 'RETRY' 버튼 'CLOSE' 버튼이 중앙에 뜹니다. 그동안 검정 배경 아래애는 음소거가 된 이벤트 영상이 계속 반복 재생됩니다.
- 모든 노드 연결 이벤트 발생 시, 메인 노드 20개의 라인으로 연결된 형태가 하트 모양으로 바뀝니다.

- 전체적인 효과음의 음량을 30% 줄입니다.
- 영상 프레임을 넘기는 스크롤 속도가 빨라질수록 효과음의 피치가 높아집니다.